...
TARGETS = ["temp"]   # Use temp only for scaling experiment (fastest)

TRAIN_CONFIG = dict(
    epochs=10,        # Reduced for scaling experiment (full=30)
    lr=3e-4,
    patience=5,
    lam_s=0.05,
    lam_a=0.01,
...
def train_model_on_gpu(arch, target, config, scalers_ref, fi_ref,
    ...
    # Dereference Ray object store
    SC = ray.get(scalers_ref)
    FI = ray.get(fi_ref)
...
# Model-target pairs for experiment (temp only for speed)
PAIRS = [(arch, "temp") for arch in ARCHES]
N_PAIRS = len(PAIRS)
...
print(f"Models to train   : {N_PAIRS} ({ARCHES})")
print(f"Epochs per model  : {TRAIN_CONFIG['epochs']} (reduced for scaling)")
...
baseline_1gpu = [r for r in model_results if r.get("status")=="OK"]
