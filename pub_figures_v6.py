...
if not bl_path.exists():
    ...
    for tgt in ["temp","smap","moist"]:
        bl_sub = bl_df[bl_df["Target"]==tgt].copy() if "Target" in bl_df.columns else bl_df[bl_df["target"]==tgt].copy()
...
# PERF_14 — Scatter plot: Predicted vs Observed (real data points)

# ══════════════════════════════════════════════════════════════════════════════

print("\n  PERF_14: Scatter plot predicted vs observed...")
...
try:
    ...
    raw_df2 = pd.read_csv(PREPROC2/"master_processed.csv", parse_dates=["time_utc"])
    ...
    SITE_COLORS2 = {"Bedrock":"#3498DB","Transition":"#E67E22",

                    "Upland":"#27AE60","Wetland":"#E74C3C"}

    TGT_UNITS2 = {"temp":"°C","smap":"K","moist":"m³/m³"}

    TGT_LABELS2 = {"temp":"Weather Temp (°C)","smap":"SMAP Temp L1 (K)",

                   "moist":"Soil Moisture (m³/m³)"}

    TGT_RAW_MAP2 = {"temp":FI2["TEMP_TARGETS"],"smap":FI2["SMAP_TARGETS"],

                    "moist":FI2["MOIST_TARGETS"]}



    CYCLICAL2=[c for c in raw_df2.columns if any(c.startswith(p) for p in ["sin_","cos_"])]

    SNAP2=FI2["SNAP_FEATURES"]

    CORE2=[f for f in SNAP2 if f not in CYCLICAL2 and f in raw_df2.columns]

    APPROX2=[f"{t}_approx" for t in ALL_TGTS2 if f"{t}_approx" in raw_df2.columns]

    RESIDUAL2=[f"{t}_residual" for t in ALL_TGTS2 if f"{t}_residual" in raw_df2.columns]
    ...
    for feat in CORE2[:8]:
        ...
        if vc not in raw_df2.columns: raw_df2[vc]=np.where(raw_df2[feat].isna(),1.0,0.01)

        UNC2.append(vc)

    V6F2=list(dict.fromkeys(CORE2+APPROX2+RESIDUAL2+UNC2))

    V6F2=[f for f in V6F2 if f in raw_df2.columns]



    # Build graph

    coords2=LOCS2[["Latitude","Longitude"]].values.astype(np.float32)

    sc2_=coords2*np.array([111.0,63.0])

    tree2_=cKDTree(sc2_); d2_,i2_=tree2_.query(sc2_,k=7)

    sig2_=np.median(d2_[:,1:])+1e-8

    A2_np=np.zeros((N_L2,N_L2),dtype=np.float32)

    for i in range(N_L2):

        for jp in range(1,d2_.shape[1]):

            j=i2_[i,jp]; w=float(np.exp(-d2_[i,jp]/sig2_))

            A2_np[i,j]+=w; A2_np[j,i]+=w

    A2_np+=np.eye(N_L2); D2_=A2_np.sum(1,keepdims=True)**0.5

    A2_norm=torch.tensor((A2_np/(D2_*D2_.T+1e-8)).astype(np.float32))

    LB2=24



    res2=pd.read_csv(RESULTS/"v6_results_corrected.csv")



    for tgt in ["temp","smap","moist"]:

        sub2=res2[res2["Target"]==tgt]

        if sub2.empty or "Space_R2" not in sub2.columns: continue

        best_arch=sub2.loc[sub2["Space_R2"].idxmax(),"Model"]

        ckpt_p=PROJECT2/"models_v6"/"dl"/f"{best_arch}_{tgt}_v6_best.pt"

        if not ckpt_p.exists(): print(f"    ✗ {ckpt_p} missing"); continue



        raw_cols2=TGT_RAW_MAP2[tgt]

        res_cols2=[f"{c}_residual" for c in raw_cols2 if f"{c}_residual" in raw_df2.columns]

        use_cols2=res_cols2 if res_cols2 else [c for c in raw_cols2 if c in raw_df2.columns]

        approx2=[f"{c}_approx" for c in raw_cols2 if f"{c}_approx" in raw_df2.columns]

        if not use_cols2: continue



        tr2=raw_df2[raw_df2["split"]=="train"]
        ...
        test_df2=raw_df2[raw_df2["split"]=="test"].copy()

        all_ts2=sorted(test_df2["time_utc"].unique()); T2=len(all_ts2)

        ts_to_i2={t:i for i,t in enumerate(all_ts2)}

        test_df2["_ti"]=test_df2["time_utc"].map(ts_to_i2)

        test_df2["_ni"]=[loc_to_idx2.get((float(la),float(lo)))

                          for la,lo in zip(test_df2["Latitude"],test_df2["Longitude"])]

        test_df2=test_df2.dropna(subset=["_ti","_ni"])

        test_df2["_ti"]=test_df2["_ti"].astype(int)

        test_df2["_ni"]=test_df2["_ni"].astype(int)



        Xf2=np.zeros((T2,N_L2,len(V6F2)),dtype=np.float32)

        yf2=np.zeros((T2,N_L2,len(use_cols2)),dtype=np.float32)

        af2=np.zeros((T2,N_L2,max(len(approx2),1)),dtype=np.float32)

        Xf2[test_df2["_ti"].values,test_df2["_ni"].values]=feat_sc2.transform(test_df2[V6F2].fillna(0).values).astype(np.float32)

        yf2[test_df2["_ti"].values,test_df2["_ni"].values]=tgt_sc2.transform(test_df2[use_cols2].fillna(0).values).astype(np.float32)

        if approx2:

            af2[test_df2["_ti"].values,test_df2["_ni"].values]=test_df2[approx2].fillna(0).values.astype(np.float32)
        ...
        try:
            ...
            exec(open(PROJECT2/"train_soil_spatial_v6.py").read().split("if args.mode")[0],exec_ns2)

            arch_cls2=exec_ns2.get("MODEL_MAP",{}).get(best_arch)

        except: arch_cls2=None

        if arch_cls2 is None: print(f"    ✗ arch class not found"); continue



        model2=arch_cls2(nf=len(V6F2),h=96,nl=2,gl=2,nt=len(use_cols2))

        model2.load_state_dict(ckpt.get("model_state_dict",ckpt.get("state_dict",{})),strict=False)

        model2.eval()



        site_true={s:[] for s in SITES2}

        site_pred={s:[] for s in SITES2}

        preds2=np.full((T2,N_L2),np.nan)

        trues2=np.full((T2,N_L2),np.nan)



        with torch.no_grad():

            for ti in range(LB2,T2,4):

                Xw2=torch.tensor(Xf2[ti-LB2:ti]).unsqueeze(0)

                out2=model2(Xw2,A2_norm.unsqueeze(0))

                mu2=out2[0] if isinstance(out2,tuple) else out2

                mu_np2=tgt_sc2.inverse_transform(

                    mu2[0].float().numpy().reshape(-1,len(use_cols2))).reshape(N_L2,len(use_cols2))

                y_np2=tgt_sc2.inverse_transform(

                    yf2[ti].reshape(-1,len(use_cols2))).reshape(N_L2,len(use_cols2))

                av2=af2[ti,:,0] if approx2 else np.zeros(N_L2)

                preds2[ti]=mu_np2[:,0]+av2

                trues2[ti]=y_np2[:,0]+av2



        times2=np.array(all_ts2)
        ...
        for ai,site in enumerate(["Bedrock","Transition","Upland","Wetland"]):
            ...
            ax=axes_flat[ai]; color=SITE_COLORS2[site]

            sr=raw_df2[raw_df2["Site"]==site][["Latitude","Longitude"]].drop_duplicates()

            sl=[loc_to_idx2.get((float(r.Latitude),float(r.Longitude)))

                 for _,r in sr.iterrows()

                 if loc_to_idx2.get((float(r.Latitude),float(r.Longitude))) is not None]

            if not sl: continue

            yt=np.nanmean(trues2[:,sl],axis=1); yp=np.nanmean(preds2[:,sl],axis=1)
            ...
            ax.scatter(yt,yp,c=color,alpha=0.35,s=10,edgecolors="none",rasterized=True)

            lim=[min(yt.min(),yp.min())-0.3,max(yt.max(),yp.max())+0.3]

            ax.plot(lim,lim,"k-",lw=2,alpha=0.7,label="1:1 line")

            z=np.polyfit(yt,yp,1); p=np.poly1d(z)

            xf=np.linspace(lim[0],lim[1],100)

            ax.plot(xf,p(xf),"--",color=color,lw=1.8,alpha=0.85,label="Best fit")

            ax.set_xlim(lim); ax.set_ylim(lim)

            ax.set_xlabel(f"Observed ({TGT_UNITS2[tgt]})",fontsize=11)

            ax.set_ylabel(f"Predicted ({TGT_UNITS2[tgt]})",fontsize=11)

            unseen=""

            if site=="Wetland":

                ax.set_facecolor("#FFF5F5"); unseen=" (UNSEEN)"

            ax.set_title(f"{site}{unseen}  |  R²={r2:.4f}  |  Bias={bias:+.3f}",

                          fontweight="bold",color=color,fontsize=12)

            ax.text(0.05,0.93,f"n={len(yt):,}",transform=ax.transAxes,

                     fontsize=9,color="grey")

            ax.legend(fontsize=9)

        fig.suptitle(f"Predicted vs Observed | {TGT_LABELS2[tgt]} | {best_arch}\n"

                      f"All 4 Sites | Test 2025 | Wetland = unseen holdout",

                      fontsize=14,fontweight="bold")

        plt.tight_layout()

        plt.savefig(FIGS/f"PERF_14_scatter_{tgt}.png",dpi=300,bbox_inches="tight")

        plt.close()

        print(f"    ✓ PERF_14_scatter_{tgt}.png  [{best_arch}]")
        ...
        for ai,site in enumerate(["Bedrock","Transition","Upland","Wetland"]):
            ...
            ax=axes[ai]; color=SITE_COLORS2[site]
            ...
            yt=np.nanmean(trues2[:,sl],axis=1)

            yp=np.nanmean(preds2[:,sl],axis=1)

            mk=~(np.isnan(yt)|np.isnan(yp))

            r2=float(1-np.sum((yt[mk]-yp[mk])**2)/(np.sum((yt[mk]-yt[mk].mean())**2)+1e-10)) if mk.sum()>5 else np.nan

            tm_=ckpt.get("test_metrics",{})

            ubr=tm_.get("std_test",{}).get("seen_ubRMSE",0.5)

            ax.fill_between(times2,yp-ubr,yp+ubr,alpha=0.15,color=color,label="±1 ubRMSE")

            ax.plot(times2,yt,color=color,lw=2.0,alpha=0.9,label="Observed")

            ax.plot(times2,yp,"k--",lw=1.8,alpha=0.85,label="Predicted")

            ax.axhline(0,color="grey",lw=0.8,ls=":",alpha=0.5)

            unseen=" (UNSEEN — holdout)" if site=="Wetland" else ""

            ax.set_ylabel(TGT_UNITS2[tgt],fontsize=11)

            ax.set_title(f"{site}{unseen}  |  R²={r2:.4f}",

                          fontweight="bold",color=color,fontsize=12,loc="left",pad=4)
        ...
        fig.suptitle(f"Actual vs Predicted {TGT_LABELS2[tgt]} | {best_arch}\n"

                      f"All 4 Sites | Mean over locations | Test 2025 | Shaded = ±1 ubRMSE",

                      fontsize=14,fontweight="bold",y=1.00)

        plt.savefig(FIGS/f"PERF_15_temporal_{tgt}.png",dpi=300,bbox_inches="tight")

        plt.close()

        print(f"    ✓ PERF_15_temporal_{tgt}.png  [{best_arch}]")
    ...
    print(f"    ✗ PERF_14/15: {e}")

    traceback.print_exc()



print("\nDone. All figures complete.")
