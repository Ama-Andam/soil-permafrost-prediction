...
# Use Ray for parallel execution if available
if ray_ok and not ray.is_initialized():
    n_gpus = int(os.environ.get("SLURM_GPUS_ON_NODE", 1))
    ray.init(num_gpus=n_gpus, num_cpus=int(os.environ.get("SLURM_CPUS_ON_NODE", 8)),
             ignore_reinit_error=True, logging_level="WARNING")
    print(f"  Ray initialised: {ray.available_resources()}")

    @ray.remote(num_gpus=1, num_cpus=2, max_retries=1)
    def ray_train(arch, tgt, seed):
        import ray as _ray  # worker needs own import
        r2_s, r2_u, elapsed, _ = train_and_eval(arch, tgt, seed=seed, epochs=20)
        return dict(arch=arch, tgt=tgt, seed=seed,
                    r2_seen=r2_s, r2_unseen=r2_u, elapsed_s=elapsed)

    # Dispatch all 110 jobs (11 models × 10 seeds) in batches of n_gpus
    all_jobs = [(arch, seed) for arch in ARCHES for seed in range(N_SEEDS)]
    results_flat = []
    for i in range(0, len(all_jobs), n_gpus):
        batch = all_jobs[i:i + n_gpus]
        futures = [ray_train.remote(arch, STAB_TGT, seed)
                   for arch, seed in batch]
        batch_res = ray.get(futures)
        results_flat.extend(batch_res)
        for r in batch_res:
            print(f"  [{r['arch']:<20}] seed={r['seed']} "
                  f"R²_seen={r['r2_seen']:.4f} "
                  f"R²_unseen={r['r2_unseen']:.4f} | {r['elapsed_s']:.0f}s")

    for r in results_flat:
        stab_rows.append(dict(
            Model=r["arch"], Target=STAB_TGT, Tier=TIERS.get(r["arch"], "?"),
            Seed=r["seed"], R2_Seen=r["r2_seen"], R2_Unseen=r["r2_unseen"],
    ...
    for arch in ARCHES:
        for seed in range(N_SEEDS):
            print(f"  {arch:<20} seed={seed}", end=" ", flush=True)
            r2_s, r2_u, elapsed, _ = train_and_eval(arch, STAB_TGT, seed=seed, epochs=20)
            stab_rows.append(dict(
                Model=arch, Target=STAB_TGT, Tier=TIERS.get(arch, "?"),
                Seed=seed, R2_Seen=r2_s, R2_Unseen=r2_u, Elapsed_s=elapsed))
            print(f"R²_seen={r2_s:.4f} R²_unseen={r2_u:.4f} | {elapsed:.0f}s")
